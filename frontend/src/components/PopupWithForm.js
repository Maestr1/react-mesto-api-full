...
export default function PopupWithForm({ isLoading, onSubmit, name, isOpen, title, onClose, buttonText, children }) {
  ...
  return (<div onClickCapture={ e => (e.currentTarget === e.target) && onClose() }
               className={ `popup popup_type_${ name } ${ isOpen ? 'popup_opened' : '' }` }>
    <div className="popup__container">
      <h2 className="popup__title">{ title }</h2>
      <form onSubmit={ onSubmit } className="popup__form" noValidate>
        { children }
        <button ref={ btnRef } className="popup__submit-btn"
                type="submit">{ isLoading ? 'Загрузка...' : buttonText }</button>
      </form>
